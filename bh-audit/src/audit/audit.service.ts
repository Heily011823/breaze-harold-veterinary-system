...
export class AuditService {
  ...
  async findAll(filters: FindEventsFilter): Promise<PaginatedResult<AuditEvent>> {
    const {
      actionType,
      userId,
      userRole,
    ...
    if (limit < 1 || limit > 100) throw new BadRequestException('limit debe estar entre 1 y 100');
    ...
    const query: SelectQueryBuilder<AuditEvent> = this.auditEventRepository
    ...
    if (startDate) {
      query.andWhere('event.occurredAt >= :startDate', { startDate: new Date(startDate) });
    }
    if (endDate) {
      query.andWhere('event.occurredAt <= :endDate', { endDate: new Date(endDate) });
    }
